...
class Linear(Module):
    ...
    def forward(self, A):
        ...
        return np.transpose(A.T @ self.W) + self.W0  # Your code (n x b)
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        return dLdA * (1 - self.A**2)  # Your code: return dLdZ (n, b)


class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A = np.where(Z <= 0, 0, Z)  # Your code: (n, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        return dLdA * (self.A != 0)  # Your code: return dLdZ (n, b)


class SoftMax(Module):  # Output activation
    def forward(self, Z):
        exp_Z = np.exp(Z)
        return exp_Z / np.sum(exp_Z, axis=0) # Your code: (n, b)

    def backward(self, dLdZ):  # Assume that dLdZ is passed in
        return dLdZ

    def class_fun(self, Ypred):  # Return class indices (n, b)
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        self.Ypred = Ypred  # (b, 1) [.2, .4, .7, .9, .6, .3]
        self.Y = Y  # (b, 1) [0, 0, 1, 1, 0, 1]
        return -np.sum(Y * np.log(Ypred))  # Your code: return loss (scalar)

    def backward(self):  # Use stored self.Ypred, self.Y
        return self.Ypred - self.Y  # Your code (1, b)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            dLdZ_L = self.loss.backward()
            self.backward(dLdZ_L)
            self.sgd_step(lrate)
