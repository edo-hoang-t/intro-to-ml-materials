...
def perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        for i in range(n):
            y_i = labels[0][i]
            if (np.dot(np.transpose(theta), data[:, [i]]) + theta0) * y_i <= 0:
                theta += y_i * data[:, [i]]
                theta0 += y_i
    return (theta, theta0)
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        sum_accur += eval_classifier(learner, data_train, labels_train, data_train, labels_train)
...
def xval_learning_alg(learner, data, labels, k):
    sum_score = 0.0
    D_splitted = np.array_split(data, k, axis=1)
    labels_splitted = np.array_split(labels, k, axis=1)
    for i in range(k):
        D_left = D_splitted[0:i]
        D_right = D_splitted[(i+1):k]
        labels_left = labels_splitted[0:i]
        labels_right = labels_splitted[(i+1):k]
        
        D_minus_i = np.concatenate(D_left + D_right, axis=1)
        labels_minus_i = np.concatenate(labels_left + labels_right, axis=1)
...
score_sum = 0
for i in range(50):
    score_sum = score_sum + eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5)
print(score_sum / 50)

score_sum = 0
for i in range(50):
    score_sum = score_sum + eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 5)
print(score_sum / 50)
